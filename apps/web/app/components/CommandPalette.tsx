  ...
  return (
    ...
    >
      <div
        ...
        className="w-full max-w-[34rem] border border-ink bg-sheet"
      >
        <input
          ref={input}
          value={query}
          ...
          className="w-full border-b border-rule bg-sheet px-4 py-3 text-body text-ink placeholder:text-ink-faint focus:outline-none"
          onKeyDown={(event) => {
        ...
        {matches.length === 0 ? (
          <p className="px-4 py-6 text-data text-ink-soft">Nothing matches that.</p>
        ) : (
              ...
              return (
                <li key={command.id}>
                  {previous?.group !== command.group && (
                    <p className="border-b border-rule-hair bg-paper px-4 py-1 text-micro text-ink-soft">
                      {command.group}
                    </p>
                  )}
                  <button
                    ...
                    className={`flex w-full items-baseline gap-4 px-4 py-2 text-left ${
                      index === cursor ? "bg-marked" : ""
                    }`}
                  >
                    <span className="min-w-0 flex-1 truncate text-data">{command.label}</span>
                    {command.hint && (
                      <span className="tabular shrink-0 font-mono text-micro text-ink-soft">
                        {command.hint}
                      </span>
                    )}
                  </button>
...
function Key({ children }: { children: React.ReactNode }) {
  return (
    <kbd className="border border-rule bg-paper px-1 font-mono text-micro text-ink-soft">
      {children}
    </kbd>
  );
}
