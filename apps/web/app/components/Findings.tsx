  ...
  return (
    <div className="grid min-w-0 gap-0 lg:grid-cols-[minmax(0,1fr)_minmax(400px,44%)]">
      ...
      <div className="min-w-0 pr-0 lg:pr-8">
        ...
        <div className="sticky top-0 z-10 bg-paper pb-3 pt-8">
          <div className="flex flex-wrap items-baseline justify-between gap-x-6 gap-y-2">
            <h2 className="text-data font-semibold">
            ...
            >
              ...
              <button
                ...
                className="border border-rule px-2 py-1 text-micro text-ink-soft hover:border-ink hover:text-ink"
              >
                ⌘K · keys
              </button>
            </nav>
        ...
        {shown.length === 0 ? (
          <p className="ruled py-8 text-body text-ink-soft">
      ...
      {undoable && markedRisks.length === 0 && (
        <Toast onDismiss={() => setUndoable(null)}>
          ...
          <button
            type="button"
            onClick={() => void undo()}
            className="underline decoration-rule-strong underline-offset-4 hover:decoration-paper"
          >
            Undo
          </button>
          <span className="text-ink-faint">⌘Z</span>
        </Toast>
      )}
  ...
  return (
    <div
      className={`ruled group flex w-full items-stretch gap-3 transition-colors ${
        marked ? "bg-marked/50" : selected ? "bg-sheet" : "hover:bg-sheet"
      }`}
    >
      ...
      <button
        ...
        className={`no-print my-2 h-3.5 w-3.5 shrink-0 self-start border border-rule-strong text-micro leading-none text-paper transition-opacity hover:border-ink focus:opacity-100 group-hover:opacity-100 ${
      ...
      >
        ...
        <span className="flex items-baseline gap-3">
          <span className="min-w-0 flex-1 truncate text-data">{supplierOf(risk)}</span>

          {/* Never truncated, and hidden rather than clipped where there is no
              room for it — on a phone it moves to the line below. */}
          <span className="tabular hidden w-[18ch] shrink-0 break-all font-mono text-micro text-ink-soft sm:block">
            {document}
          </span>

          <span
            className="tabular shrink-0 text-data font-medium"
        ...
        <span className="mt-0.5 flex items-baseline gap-3 text-micro text-ink-soft">
          <span className="min-w-0 flex-1 truncate">
            {RULE_LABEL[risk.rule_code] ?? risk.title}
            {document && <span className="tabular ml-2 font-mono sm:hidden">{document}</span>}
            {status !== "open" && (
              <span className="ml-2 border border-rule px-1 text-ink-soft">{status}</span>
            )}
          </span>
          ...
          {amount && risk.headline_pct && (
            <span className="tabular shrink-0 text-ink-faint">{pct(risk.headline_pct)}</span>
          )}
        </span>
      </button>
    </div>
...
function BulkBar({
  ...
  return (
    <div className="no-print fixed inset-x-0 bottom-0 z-40 flex flex-wrap items-center justify-center gap-3 border-t border-ink bg-ink px-4 py-3 text-paper">
      <span className="tabular text-data">
      ...
      {canWrite ? (
        QUICK.map((entry) => (
          <button
            ...
            className="border border-paper/40 px-3 py-1 text-micro hover:bg-paper hover:text-ink disabled:opacity-50"
          >
            {entry.label} <span className="ml-1 opacity-60">{entry.key}</span>
          </button>
        ))
      ) : (
        <span className="text-micro opacity-70">This account has read-only access.</span>
      )}
      <button type="button" onClick={onClear} className="text-micro underline opacity-70">
        Clear
      </button>
    </div>
  );
...
function Toast({
  ...
  return (
    <div
      role="status"
      className="no-print fixed bottom-6 left-1/2 z-40 flex -translate-x-1/2 items-center gap-4 border border-ink bg-ink px-4 py-2 text-data text-paper"
    >
      {children}
    </div>
  );
...
function Shortcuts() {
  ...
  return (
    <dl className="mt-3 grid gap-x-6 gap-y-1 border border-rule bg-sheet px-4 py-3 text-micro sm:grid-cols-2">
