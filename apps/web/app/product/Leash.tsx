...
export function Leash() {
  return (
    <div className="mt-6 max-w-[64ch]">
      ...
      <pre className="mt-2 overflow-x-auto border border-rule bg-sheet p-3 font-mono text-micro leading-relaxed text-ink-soft">
        {TOOL_OUTPUT}
      </pre>
        ...
        {ATTEMPTS.map((attempt) => {
          ...
          return (
            <div
              ...
              className="grid gap-x-4 gap-y-1 border-b border-rule-hair py-3 sm:grid-cols-[minmax(0,1fr)_5rem]"
            >
              <dt className="text-data leading-relaxed">
                {refused ? (
                  <del className="text-ink-faint decoration-exposure decoration-1">
                    {attempt.sentence}
                  </del>
                ) : (
                  <span className="text-ink">{attempt.sentence}</span>
                )}
                <span
                  className={`mt-1 block text-micro leading-relaxed ${
                    refused ? "text-exposure" : "text-ink-faint"
                  }`}
                >
                  {attempt.reason}
                </span>
              ...
              <dd
                className={`font-mono text-micro sm:text-right ${
                  refused ? "text-exposure" : "text-reconciled"
                }`}
              >
